...
fn main(){
    ...
    while search_points.len() > 0{
        let seed = search_points.pop_front().expect("We only loop ");
        let mut visited_9: HashSet<Seed> = HashSet::new();
        let mut search_stack: Vec<Seed> = Vec::new();
        search_stack.push(seed);

        while search_stack.len() > 0 {
            let search_point = search_stack.pop().expect("We only loop on len>0");
            ...
            if current_height == 10{
                if !visited_9.contains(&search_point){
                    res += 1;
                    visited_9.insert(search_point);
                }
                continue;
            }
